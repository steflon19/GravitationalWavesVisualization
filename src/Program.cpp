...
bool Program::init()
    ...
    if(gShaderName.size() > 0) gShaderString = readFileAsString(gShaderName);
	const char* vshader = vShaderString.c_str();
	const char* fshader = fShaderString.c_str();
    const char* gshader = "";
    if(gShaderName.size() > 0) gshader = gShaderString.c_str();
	...
	if (GS >= 0 && gShaderName.size() > 0)
		...
		if (!rc)
			...
			if (isVerbose())
				...
				std::cout << "Error compiling fragment shader " << gShaderName << std::endl;
				}
			return false;
			}
		}
